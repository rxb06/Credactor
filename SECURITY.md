...
- **SEC-25/SEC-32 — Path traversal guard improvements (v2.3.1):** Both staged mode (SEC-32) and git history (SEC-25) now reject `..` as a path *component* (via `Path.parts`), not as a substring — so filenames like `secret..py` are no longer falsely skipped. Staged mode additionally wraps `Path.resolve()` in exception handling and validates resolved paths stay within the scan root. All `startswith` root-boundary checks now append `os.sep` to prevent prefix collisions (e.g. `/tmp/repo` vs `/tmp/repo_evil`).
