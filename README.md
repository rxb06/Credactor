...
- **The wheel must match the source.** On every push and before every publish, `scripts/audit_wheel.py` checks the built wheel against the git-tracked source: any file in the wheel that is not tracked, any tracked file missing from it, or no wheel at all fails the gate, so a build step cannot smuggle code into the artifact.
